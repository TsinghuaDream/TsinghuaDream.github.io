...
#此处有坑，当你部署springboot项目时有其他数据容器时需要加入--link 或者配置网域进行通信
docker run -id -p 9000:8080 appdocker run -d --name febs-server -p 8089:8089 --link mysql8:mysql8 --link redis:redis  febs
```
